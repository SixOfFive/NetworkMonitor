...
_VALID_USER_RE = re.compile(r"^[A-Za-z0-9_.][A-Za-z0-9_.\-]{0,31}$")
_VALID_IP_RE = re.compile(r"^(?:\d{1,3}\.){3}\d{1,3}$")


def build_putty_launcher_bat(user, ip):
    """Generate a Windows .bat that launches PuTTY with `-ssh user@host`.

    Avoids the ssh:// URL-handler path entirely (PuTTY's URL parsing on some
    installs doesn't strip the scheme and ends up with usernames like
    'ssh://node'). The .bat searches the standard PuTTY install paths and
    exits with a clear message if putty.exe isn't found.

    Both arguments are strictly validated before they ever land in the .bat;
    user must match _VALID_USER_RE, ip must be a dotted IPv4 string.
    """
    if not ip or not _VALID_IP_RE.match(ip):
        raise ValueError("invalid ip")
    if user and not _VALID_USER_RE.match(user):
        raise ValueError("invalid user")
    target = f"{user}@{ip}" if user else ip
    return (
        "@echo off\r\n"
        "REM Launches PuTTY with -ssh so the username is parsed correctly.\r\n"
        "REM Auto-generated by netmon for a one-click SSH session.\r\n"
        "set PUTTY=\r\n"
        'if exist "%ProgramFiles%\\PuTTY\\putty.exe" set PUTTY=%ProgramFiles%\\PuTTY\\putty.exe\r\n'
        'if exist "%ProgramFiles(x86)%\\PuTTY\\putty.exe" set PUTTY=%ProgramFiles(x86)%\\PuTTY\\putty.exe\r\n'
        'if "%PUTTY%"=="" (\r\n'
        '  echo Could not find putty.exe. Install PuTTY from https://www.putty.org and try again.\r\n'
        "  pause\r\n"
        "  exit /b 1\r\n"
        ")\r\n"
        f'start "" "%PUTTY%" -ssh {target}\r\n'
        "exit /b 0\r\n"
    )
...
class Monitor:
    def __init__(self, ips, workers, state_path=None):
        ...
        # Per-MAC ping history for the click-to-open chart modal.
        # mac -> {"samples": deque[(ts, ms, ip)], "ip_changes": list[(ts, prev_ip, new_ip)]}
        self.mac_history = {}
        ...
        with self.lock:
            mac_history_copy = {
                mac: {
                    ...
                    "ip_changes": list(h["ip_changes"]),
                }
                for mac, h in self.mac_history.items()
            }
        data = {
            "saved_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def scan_once(self):
        ...
        with self.lock:
            for ip, (ms, names, method, did_resolve) in responding.items():
                ...
                if mac:
                    self._record_ping(mac, now, ms, ip)
    ...
    def mac_snapshot(self, mac):
        """Per-MAC view: ping samples, IP changes, current device info.

        Used by /api/mac/<mac> to populate the click-to-open chart modal.
        """
        if not mac:
            return None
        mac = mac.strip().lower()
        with self.lock:
            ...
            if current is not None:
                if not current.get("present"):
                    ...
                    age = time.time() - current["last_seen"]
...
/* Click-to-open ping chart modal */
tr[data-mac], .ev[data-mac] { cursor: pointer; }
tr[data-mac]:hover td { background: #1d2630 !important; }
.ev[data-mac]:hover { background: #1a2129; }
.modal-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.78); display: none; z-index: 100; align-items: flex-start; justify-content: center; padding: 40px 20px; overflow-y: auto; }
.modal-backdrop.open { display: flex; }
.modal { background: #141a21; border: 1px solid #2a323c; border-radius: 6px; padding: 16px 20px 18px; min-width: 720px; max-width: 95vw; box-shadow: 0 8px 32px rgba(0,0,0,0.6); }
.modal-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #232a32; }
.modal-title { color: #5fb3b3; font-size: 14px; font-weight: 600; letter-spacing: 0.5px; }
.modal-title .mac { color: #c0c5ce; font-weight: normal; margin-left: 8px; }
.modal-close { background: transparent; color: #ec5f67; border: none; font-size: 22px; line-height: 1; cursor: pointer; padding: 0 4px; }
...
<div class="modal-backdrop" id="modal-backdrop">
  <div class="modal" id="modal" role="dialog" aria-modal="true">
    <div class="modal-header">
      <div class="modal-title">PING HISTORY <span class="mac" id="modal-mac"></span></div>
...
async function refresh() {
  ...
  document.getElementById('devices').innerHTML = sortDevices(s.devices).map(d => {
    ...
    const macAttr = d.mac ? ` data-mac="${esc(d.mac)}" title="Click for ping history"` : '';
    return `
    <tr class="${rowCls}"${macAttr}>
  ...
  document.getElementById('events').innerHTML = evs.map(e => {
    ...
    const macAttr = e.mac ? ` data-mac="${esc(e.mac)}" title="Click for ping history"` : '';
    return `<div class="ev ${esc(e.kind)}"${macAttr}><span class="t">[${esc(e.time)}]</span> ${sym} ${esc(pad(e.ip, 15))}  ${esc(pad(e.mac, 17))}  ${esc(e.name)}</div>`;
  }).join('');
}
refresh();
setInterval(refresh, 2000);

// ---------- Click-to-open MAC ping history modal ----------
...
function renderChart(samples, ipChanges, ipColors) {
  ...
  const W = 880, H = 280, padL = 56, padR = 16, padT = 14, padB = 30;
  const tMin = samples[0][0];
  const tMax = samples[samples.length - 1][0];
  const tRange = Math.max(tMax - tMin, 1);
  const msVals = samples.map(s => s[1]);
  const msMaxRaw = Math.max(...msVals);
  const msMax = Math.max(msMaxRaw * 1.15, 1);
  ...
  const yTicks = [0, 0.25, 0.5, 0.75, 1].map(f => {
    ...
    return `
      <line x1="${padL}" y1="${yp}" x2="${W - padR}" y2="${yp}" stroke="#1c232b" stroke-width="1"/>
      <text x="${padL - 5}" y="${(parseFloat(yp) + 3).toFixed(1)}" fill="#768390" font-size="9" text-anchor="end">${m.toFixed(1)} ms</text>
    `;
  }).join('');
  ...
  const xTicks = [0, 0.25, 0.5, 0.75, 1].map(f => {
    ...
    return `
      <line x1="${xp}" y1="${H - padB}" x2="${xp}" y2="${H - padB + 4}" stroke="#768390"/>
      <text x="${xp}" y="${H - padB + 16}" fill="#768390" font-size="9" text-anchor="middle">${fmtTsShort(t)}</text>
    `;
  }).join('');
...
async function openMacModal(mac) {
  ...
  let data;
  try {
    const r = await fetch('/api/mac/' + encodeURIComponent(mac));
...
function puttyLauncherUrl(user, ip) {
  // Server returns a .bat that calls `putty.exe -ssh user@host`. Avoids the
  // ssh:// URL-handler path because PuTTY's URL parsing on many installs
  // doesn't strip the scheme — `ssh://node@host` becomes username
  // "ssh://node" and authentication fails.
  if (!ip) return null;
  const qs = new URLSearchParams({ ip });
  if (user) qs.set('user', user);
  return '/api/ssh/launcher?' + qs.toString();
}

function puttyButton(user, ip) {
  const url = puttyLauncherUrl(user, ip);
  if (!url) {
    return `<a class="ssh-btn launch disabled" title="No IP available">Open in PuTTY</a>`;
  }
  // `download` attribute makes browsers save instead of navigate; the
  // server also sets Content-Disposition: attachment as a backstop.
  return `<a class="ssh-btn launch" href="${esc(url)}" download title="Downloads a .bat that opens PuTTY with -ssh ${esc(user || '<user>')}@${esc(ip)} — click it from your downloads tray to connect">Open in PuTTY</a>`;
}
...
document.addEventListener('click', (e) => {
  ...
  if (tr) {
    e.preventDefault();
    openMacModal(tr.dataset.mac);
    return;
  }
  const ev = e.target.closest('.ev[data-mac]');
  if (ev) {
    e.preventDefault();
    openMacModal(ev.dataset.mac);
  }
});
</script>
</body></html>
...
def make_handler(monitor):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            elif self.path.startswith("/api/ssh/launcher"):
                # GET /api/ssh/launcher?ip=...&user=... → download .bat that
                # launches PuTTY with -ssh user@host (works around URL-handler
                # registrations that mis-parse ssh:// URLs).
                qs = self.path.split("?", 1)
                params = {}
                if len(qs) == 2:
                    params = {k: v[0] for k, v in
                              urllib.parse.parse_qs(qs[1]).items()}
                ip = (params.get("ip") or "").strip()
                user = (params.get("user") or "").strip()
                try:
                    bat = build_putty_launcher_bat(user, ip)
                except ValueError as e:
                    self._send(400, json.dumps({"error": str(e)}), "application/json")
                    return
                payload = bat.encode("utf-8")
                safe_ip = ip.replace(".", "-")
                fname = f"connect-{user + '-' if user else ''}{safe_ip}.bat"
